...
namespace TarodevController
{
    ...
    public class PlayerController : MonoBehaviour, IPlayerController, IPhysicsObject
    {
        ...
        private void SetupCharacter()
        {
            ...
            _currentStepHeight = _character.StepHeight;
            _currentHeight = _character.Height;

            _cachedQueryMode = Physics2D.queriesStartInColliders;

            _wallDetectionBounds = new Bounds(
                new Vector3(0, _character.Height / 2),
                new Vector3(_character.StandingColliderSize.x - SKIN_WIDTH * 2, _character.Height - 0.1f));
        }
        ...
        private float GrounderLength => _currentStepHeight + SKIN_WIDTH;
        private const float RAY_INSET = 0.1f;
        private Vector2 RayPoint => _framePosition + Up * (_currentStepHeight + SKIN_WIDTH);
        ...
        private void ToggleGrounded(bool grounded)
        {
            ...
            SetColliderMode(grounded ? ColliderMode.Standard : ColliderMode.Airborne);
        }
        ...
        private void SetColliderMode(ColliderMode mode)
        {
            switch (mode)
            {
                ...
                case ColliderMode.Airborne:
                    // This causes slipping on slope landings and inconsistent corner slipping
                    _collider.size = new Vector2(_character.CrouchColliderSize.x, _character.Height - SKIN_WIDTH);
                    _collider.offset = new Vector2(0, _character.Height / 2 + SKIN_WIDTH);
                    break;
            }
        }
        ...
        private bool ShouldCenterOnLadder => Stats.SnapToLadders && _frameInput.Move.x == 0;
        ...
        private void CalculateJump()
        {
            ...
            _wallJumpInputNerfPoint = Mathf.MoveTowards(_wallJumpInputNerfPoint, 1, _delta / Stats.WallJumpInputLossTime);
        }
        ...
        private void ToggleCrouching(bool shouldCrouch)
        {
            if (shouldCrouch)
            {
                ...
                _currentStepHeight = _character.CrouchingStepHeight;
                _currentHeight = _character.CrouchingHeight;
            ...
            SetColliderMode(shouldCrouch ? ColliderMode.Crouching : ColliderMode.Standard);
        }
        ...
        private void TraceGround()
        {
            ...
            if (_grounded && !IsWithinJumpClearance)
            {
                ...
                var distanceFromGround = _currentStepHeight - _groundHit.distance;
        ...
        private void Move()
        {
            ...
            if (ClimbingLadder)
            {
                ...
                // Horizontal
                float goalX;
                if (ShouldCenterOnLadder && _frameInput.Move.x == 0)
                {
            ...
            else
            {
                ...
                var dir = Mathf.Lerp(-_wallDirectionForJump, xDir.x, _wallJumpInputNerfPoint);
                //var dir = _wallJumpInputNerfPoint > 0 ? -_lastWallDirection : xDir.x;

                var targetX = Mathf.MoveTowards(_trimmedFrameVelocity.x, dir * targetSpeed, step);
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawWireCube(pos + Vector2.up * ((_currentStepHeight + _currentHeight) / 2), new Vector2(_character.StandingColliderSize.x, _currentHeight - _currentStepHeight));
            Gizmos.color = Color.magenta;

            Gizmos.DrawWireCube(pos + Vector2.up * _currentStepHeight / 2, new Vector2(_character.StandingColliderSize.x, _currentStepHeight));

            var rayStart = pos + Vector2.up * _currentStepHeight;
            var rayDir = Vector3.down * _currentStepHeight;
